...
Item {
  ...
  // Strict isolation: only the dedicated Yandex Music MPRIS identity is used.
  // Regular Chromium/YouTube players never become active in this plugin.
  readonly property var players: {
    var yandex = []
    for (var i = 0; i < allPlayers.length; i++) {
      if (isYandexPlayer(allPlayers[i])) yandex.push(allPlayers[i])
    }
    return yandex
  }
  ...
  function isYandexPlayer(player) {
    if (!player) return false
    var label = [player.identity || "", player.desktopEntry || "", player.dbusName || ""]
      .join(" ").toLowerCase()
    return label.indexOf("yandex") !== -1
      || label.indexOf("yamusic") !== -1
      || label.indexOf("яндекс") !== -1
  }
