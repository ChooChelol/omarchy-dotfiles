...
BarWidget {
  id: root
  moduleName: "vv.yandex-music"

  readonly property var mediaService: bar?.shell?.serviceFor("vv.yandex-music")
  ...
  function togglePlayback() {
    var handled = root.mediaService ? root.mediaService.runAction("playPause", false) : false
    if (!handled && root.bar)
      root.bar.run("$HOME/.config/omarchy/plugins/vv.yandex-music/scripts/play-pause.sh")
  }
  ...
  MouseArea {
    ...
    onEntered: if (root.bar) root.bar.showTooltip(root, root.hasMedia ? (root.title + (root.artist ? " — " + root.artist : "")) : "Открыть Яндекс Музыку")
